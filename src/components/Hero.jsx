...
const CoffeeBean = ({ size = 38, color = '#6F4E37', opacity = 0.55 }) => (
  <svg width={size} height={size * 1.55} viewBox="0 0 38 59" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M19 5 C10 15, 10 25, 19 29.5 C28 34, 28 44, 19 54"
      stroke="white" strokeWidth="1.8" strokeLinecap="round" opacity="0.35" fill="none" />
  </svg>
);
...
const Hero = () => {
  ...
  // Keyboard: Escape closes the book
  useEffect(() => {
  ...
  return (
    <>
      ...
      {menuOpen && <MenuBook onClose={() => setMenuOpen(false)} />}

      <section className="relative min-h-[90vh] flex items-center overflow-hidden" style={{ background: '#F5F1EA' }}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-14 w-full grid grid-cols-1 md:grid-cols-2 gap-12 py-20">
          ...
          <div className="flex flex-col justify-center gap-8">
            ...
            <div className="flex items-center gap-3 transition-all duration-700"
              ...
              <p className="text-xs font-bold tracking-[0.3em] uppercase text-[#6F4E37]">Est. 2024 · Bhopal's Finest</p>
            </div>
            ...
            <p className="text-base text-[#6F4E37]/80 max-w-sm leading-relaxed transition-all duration-700"
              ...
              Handcrafted coffees and desserts made with love. Every cup tells a story — come find yours.
            </p>
            ...
            <div className="flex items-center gap-4 transition-all duration-700"
              ...
              {/* ── Explore Menu → opens flipbook ── */}
              <button
                onClick={() => setMenuOpen(true)}
                className="group relative bg-[#2C1810] text-[#F5F1EA] font-bold py-3.5 px-8 rounded-full
                  hover:bg-[#6F4E37] transition-all duration-300 text-sm tracking-widest uppercase
                  overflow-hidden"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 text-sm font-semibold text-[#2C1810]
              ...
              {categories.map(({ icon, label }) => (
                <div key={label}
                  className="flex items-center gap-2 bg-white/70 backdrop-blur-sm border border-[#C4B5A0]/50
                    px-4 py-2 rounded-full cursor-pointer hover:bg-[#2C1810] hover:border-[#2C1810]
                    transition-all duration-200 group">
                  <span className="text-base">{icon}</span>
                  <span className="text-xs font-bold tracking-widest uppercase text-[#2C1810] group-hover:text-white">{label}</span>
                </div>
              ))}
            </div>
          </div>
